...
function Cardproduct ({information}) {
    return (
        <div className="rounded-2xl shadow-md overflow-hidden bg-white">
            ...
            <div className="p-4">
                <h3 className="text-xl font-medium mb-3 hover:text-[#C122ED]"><Link to={`/artistas/${information?.artista_slug || information?.artista?.slug }`}>{information?.artista_nombre || information?.artista?.nombre || 'The Rolling Stones'}</Link></h3>
